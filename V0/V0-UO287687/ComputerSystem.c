...
void ComputerSystem_PowerOn(int argc, char *argv[]) {
  int i, nm;
	...
	FILE *nameOfTheProgramToBeExecuted= fopen("nameOfTheProgramToBeExecuted", "r");
	if(nameOfTheProgramToBeExecuted==NULL){
		...
		while (fgets(lineRead, MSGMAXIMUMLENGTH, nameOfTheProgramToBeExecuted) != NULL) {	
			programName=strtok(lineRead,",");
        	aux=fopen(programName,"r");
			if(aux!=NULL){
				programFile=aux;
			}
	}
