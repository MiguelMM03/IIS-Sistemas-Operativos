...
void OperatingSystem_PrintExecutingProcessInformation(){   // V2-studentsCode
	...
	if (executingProcessID>=0)
		...
		ComputerSystem_DebugMessage(78,SHORTTERMSCHEDULE,
  ...
  for (i=0; i< PROCESSTABLEMAXSIZE; i++) {
  	if (processTable[i].busy) {
		...
		if (processTable[i].state==EXIT) 
			ComputerSystem_DebugMessage(99,SHORTTERMSCHEDULE," Zombie process!");
		ComputerSystem_DebugMessage(100,SHORTTERMSCHEDULE,"\n");
  	}
  }
  if (count==0){
  	ComputerSystem_DebugMessage(100,SHORTTERMSCHEDULE,"\t\t[--- NO process association ---]");
	ComputerSystem_DebugMessage(100,SHORTTERMSCHEDULE,"\n");
  }
}
